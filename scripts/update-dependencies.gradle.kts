...
tasks.register("updateDependencies") {
    ...
    doLast {
        ...
        val report = java.io.File(reportPath)
        if (!report.exists()) {
            println("Dependency updates report not found at ${report.path}")
            return@doLast
        }

        @Suppress("UNCHECKED_CAST")
        val json = JsonSlurper().parseText(report.readText()) as Map<String, Any>
        @Suppress("UNCHECKED_CAST")
